...
export function AnalyticsClient() {
  ...
  const { data: revenueData, isLoading: isLoadingRevenue } =
    useRevenueAnalytics(parseInt(days));
    ...
    <div className="space-y-8">
      <div className="flex justify-end">
        <Select value={days} onValueChange={(val) => val && setDays(val)}>
          ...
          <SelectContent>
            <SelectItem value="7">
              {t("lastDays", { defaultMessage: "Last {days} days", days: 7 })}
            </SelectItem>
            <SelectItem value="30">
              {t("lastDays", { defaultMessage: "Last {days} days", days: 30 })}
            </SelectItem>
            <SelectItem value="90">
              {t("lastDays", { defaultMessage: "Last {days} days", days: 90 })}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader>
            <CardTitle>
              {t("revenueOverview", { defaultMessage: "Revenue Overview" })}
            </CardTitle>
            <CardDescription>
              {t("dailyRevenueDesc", {
                defaultMessage: "Daily revenue for the last {days} days",
                days,
              })}
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("topProductsDesc", {
                defaultMessage: "Highest revenue generating products",
              })}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingProducts ? (
              ...
              <div className="h-[300px] flex items-center justify-center text-muted-foreground border rounded-xl bg-muted/20">
                {t("noProductData", {
                  defaultMessage: "No product data available",
                })}
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {product.totalSold}{" "}
                          {t("unitsSold", { defaultMessage: "units sold" })}
                        </p>
                      </div>
                    </div>
